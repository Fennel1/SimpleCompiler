...
void run(Ui::MainWindow *ui, string file)
{
...
//    cout << str << endl;
    ...
    if (flag){
//        cout << "语法分析成功" << endl;
        QPalette pa = ui->textBrowser_13->palette();
        pa.setColor(QPalette::Base,QColor(0,255,0));
        ui->textBrowser_13->setPalette(pa);
    }
    else{
        cout << token.type << " " << token.value << endl;
        QPalette pa = ui->textBrowser_13->palette();
        pa.setColor(QPalette::Base,QColor(255,0,0));
        ui->textBrowser_13->setPalette(pa);
        ui->textBrowser_13->setFont(QFont(("Consolas"), 15));
        ui->textBrowser_13->insertPlainText(QString::fromStdString(ErrorMessage));
//        cout << "语法分析失败" << endl;
        return;
    }
...
//    show_QT();          //显示四元式
    QStringList slist;
    slist << QString("名称\t\t类型\t\t种类\t\t地址");
    for (int i=0; i<SYNBL.size(); i++){
        slist << QString::fromStdString(SYNBL[i].name) + "\t\t" + QString::number(SYNBL[i].type) + "\t\t" + QString::fromStdString(SYNBL[i].cat) + "\t\t" + QString::number(SYNBL[i].addr);
    }
    QStringListModel *model = new QStringListModel(slist);
    ui->listView->setModel(model);

    slist.clear();
    slist << QString("类型\t\t\t\t地址/长度");
    for (int i=0; i<TYPEL.size(); i++){
        slist << QString::fromStdString(TYPEL[i].type) + "\t\t\t\t" + QString::number(TYPEL[i].tpoint);
    }
    model = new QStringListModel(slist);
    ui->listView_2->setModel(model);

    slist.clear();
    slist << QString("参数数量\t\t\t地址");
    for (int i=0; i<PFINFL.size(); i++){
        slist << QString::number(PFINFL[i].fn) + "\t\t\t\t" + QString::number(PFINFL[i].param);
    }
    model = new QStringListModel(slist);
    ...
    for (int i=0; i<PARAML.size(); i++){
        slist << QString::fromStdString(PARAML[i].name) + "\t\t\t\t" + QString::number(PARAML[i].type);
    }
    model = new QStringListModel(slist);
    ui->listView_4->setModel(model);

    slist.clear();
    slist << QString("长度\t\t类型\t\t元素长度");
    for (int i=0; i<AINFL.size(); i++){
        slist << QString::number(AINFL[i].len) + "\t\t" + QString::number(AINFL[i].type) + "\t\t" + QString::number(AINFL[i].type_len);
    }
    model = new QStringListModel(slist);
    ui->listView_5->setModel(model);

    slist.clear();
    slist << QString("长度\t\t区距\t\t类型");
    for (int i=0; i<RINFL.size(); i++){
        slist << QString::fromStdString(RINFL[i].name) + "\t\t" + QString::number(RINFL[i].off) + "\t\t" + QString::number(RINFL[i].type);
    }
    model = new QStringListModel(slist);
...
//    }
    ...
    // show_QT();          //显示四元式
    slist.clear();
    slist << QString("i\top\t\tnum1\t\tnum2\t\tres");
    for (int i=0; i<QT.size(); i++){
        QString str="";
        str += QString::number(i+1);
        str += "\t";
        str += QString::fromStdString(QT[i].op);
        if (QString::fromStdString(QT[i].op).length() > 6)    str += "\t";
        else    str += "\t\t";
        str += QString::fromStdString(QT[i].num1);
        if (QString::fromStdString(QT[i].num1).length() > 6)    str += "\t";
        else    str += "\t\t";
        str += QString::fromStdString(QT[i].num2);
        if (QString::fromStdString(QT[i].num2).length() > 6)    str += "\t";
        else    str += "\t\t";
        str += QString::fromStdString(QT[i].res);
        slist << str;
    }
    model = new QStringListModel(slist);
    ui->listView_8->setModel(model);
...
//        show_QT_EASY(); //显示优化后的四元式
    ...
    slist.clear();
    slist << QString("i\top\t\tnum1\t\tnum2\t\tres");
    for (int i=0; i<QT_ACT.size(); i++){
        QString str="";
        str += QString::number(i+1);
        str += "\t";
        str += QString::fromStdString(QT_ACT[i].op);
        if (QString::fromStdString(QT_ACT[i].op).length() > 6)    str += "\t";
        else    str += "\t\t";
        str += QString::fromStdString(QT_ACT[i].num1.name);
        if (QString::fromStdString(QT_ACT[i].num1.name).length() > 6)    str += "\t";
        else    str += "\t\t";
        str += QString::fromStdString(QT_ACT[i].num2.name);
        if (QString::fromStdString(QT_ACT[i].num2.name).length() > 6)    str += "\t";
        else    str += "\t\t";
        str += QString::fromStdString(QT_ACT[i].res.name);
        slist << str;
    }
    model = new QStringListModel(slist);
    ui->listView_9->setModel(model);

    GetACT();       //获取活跃信息
//    show_ACT();     //显示带活跃信息的四元式

    slist.clear();
    slist << QString("op\t\tnum1\t\tnum2\t\tres");
    for (int i=0; i<QT_ACT.size(); i++){
        QString str="";
        str += QString::fromStdString(QT_ACT[i].op);
        if (QT_ACT[i].op == "elifend")  str += "\t\t";
        else if (QString::fromStdString(QT_ACT[i].op).length() >= 7)    str += "\t";
        else    str += "\t\t";
        str += QString::fromStdString(QT_ACT[i].num1.name);
        str += "(" + QString::number(QT_ACT[i].num1.active) + ")";
        if (QString::fromStdString(QT_ACT[i].num1.name).length()+QString::number(QT_ACT[i].num1.active).length() > 5)    str += "\t";
        else    str += "\t\t";
        str += QString::fromStdString(QT_ACT[i].num2.name);
        str += "(" + QString::number(QT_ACT[i].num2.active) + ")";
        if (QString::fromStdString(QT_ACT[i].num2.name).length()+QString::number(QT_ACT[i].num2.active).length() > 5)    str += "\t";
        else    str += "\t\t";
        str += QString::fromStdString(QT_ACT[i].res.name);
        str += "(" + QString::number(QT_ACT[i].res.active) + ")";
        slist << str;
    }
    model = new QStringListModel(slist);
    ui->listView_10->setModel(model);
...
//    show_OBJ();     //显示目标代码
    ...
    for (int i=0; i<OBJ.size(); i++){
        QString str="";
        str += QString::number(OBJ[i].id);
        str += "\t";
        str += QString::fromStdString(OBJ[i].op);
        if (OBJ[i].op == "elifend")  str += "\t\t";
        else if (QString::fromStdString(OBJ[i].op).length() >= 7)    str += "\t";
        else    str += "\t\t";
        str += QString::fromStdString(OBJ[i].num1);
        if (QString::fromStdString(OBJ[i].num1).length() >= 7)    str += "\t";
        else    str += "\t\t";
        str += QString::fromStdString(OBJ[i].num2);
        slist << str;
    }
    model = new QStringListModel(slist);
    ui->listView_11->setModel(model);

    WriteASM();

}

void items_clear(Ui::MainWindow *ui)
{
    str.clear();
    t_str.clear();
    while(!SEM.empty()) SEM.pop();
    while(!OBJ_SEM.empty()) OBJ_SEM.pop();
    T_type.clear();
    QT.clear();
    QT_ACT.clear();
    QT_ACT_EASY.clear();
...
void MainWindow::on_pushButton_clicked()
{
    ...
    QFile f("../" + file + ".txt");
    if (f.exists()) run(ui, file.toStdString());
